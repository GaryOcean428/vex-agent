...
# bitsandbytes: required for 4-bit quantization (NF4) so GLM-4.7-Flash
# (30B total params) fits in A10G VRAM (24GB) with headroom.
ml_image = (
        ...
        for batch_start in range(0, len(texts), batch_size):
            ...
            for text in batch:
                try:
                    ...
                    # Cast to float32 before softmax + numpy conversion.
                    # bfloat16 has no numpy dtype — direct .numpy() raises
                    # "Got unsupported ScalarType BFloat16".
                    # 4-bit models compute in float16, but compute_dtype may
                    # still return bfloat16 on some layers. .float() is safe
                    # in all cases and adds negligible overhead.
