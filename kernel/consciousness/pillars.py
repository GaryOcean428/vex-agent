...
# Tolerance for identity drift from frozen reference.
# Tacking oscillation (kappa ~55-73) naturally moves the basin
# by ~0.19 from frozen identity — healthy regime flex, not drift.
# Previous value (0.1) pre-dated active tacking, spamming every cycle.
# Warning at 0.25, critical at 0.4 (genuine dissolution).
...
class QuenchedDisorder:
    ...
    def _crystallize(self) -> None:
        ...
        Uses incremental Frechet mean on the simplex: for each basin
    ...
    def check_drift(self, current_basin: Basin) -> PillarStatus:
        ...
        if drift > IDENTITY_DRIFT_CRITICAL:
            ...
            logger.error(
                "Pillar 3: CRITICAL identity drift %.4f exceeds %.4f",
                drift,
                IDENTITY_DRIFT_CRITICAL,
            )
        elif drift > IDENTITY_DRIFT_TOLERANCE:
            # Warning-level drift — log sparingly (every 50 cycles)
            violations.append(PillarViolation.IDENTITY_DRIFT)
            corrections.append(
