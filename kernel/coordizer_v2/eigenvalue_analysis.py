...
def run_synthetic_harvest() -> "HarvestResult":
    ...
    (source_dim >> target_dim). vocab_size=64 is trivial because
    source_dim == target_dim makes PGA an identity-like operation.
    """
...
def run_compression(harvest: "HarvestResult") -> "CompressionResult":
    ...
    on the tangent-space covariance matrix (symmetric PSD). They are
    stored in CompressionResult.eigenvalues -- do NOT recompute them.
    """
...
def analyse_eigenvalues(result: "CompressionResult") -> dict:
    ...
    which were computed by compress() via np.linalg.eigh on the
    ...
    # Verify eigenvalues are real (sanity check against prior bug)
    if np.iscomplexobj(eigenvalues):
        logger.error(
            "COMPLEX eigenvalues detected -- this means compress() "
            "decomposed the wrong matrix. Expected real non-negative "
            "eigenvalues from np.linalg.eigh on symmetric Gram matrix."
        )
        return {"error": "Complex eigenvalues (wrong matrix decomposed)"}

    total = float(np.sum(eigenvalues))
    cumulative = np.cumsum(eigenvalues) / total if total > 1e-12 else np.zeros_like(eigenvalues)

    # E8 hypothesis score (already implemented in CompressionResult)
    ...
    # Spectral gap (ratio of largest to 8th eigenvalue)
    spectral_gap = (
        float(eigenvalues[0] / eigenvalues[min(7, len(eigenvalues) - 1)])
        if len(eigenvalues) > 7 and eigenvalues[min(7, len(eigenvalues) - 1)] > 1e-12
        else 0.0
    )

    # Decision gate for lens intermediate dimension
    if e8_score > 0.85:
        recommended_n = 8
...
async def main():
    ...
    # Step 2: Compress (eigenvalues computed inside compress() via eigh)
    compression = run_compression(harvest)

    # Step 3: Analyse eigenvalues (read from compression.eigenvalues)
    ...
    # Save PrincipalDirectionBank artifact if available
    if compression.principal_direction_bank is not None:
        bank_path = output_path.parent / "principal_direction_bank"
        compression.principal_direction_bank.save(str(bank_path))
        logger.info("PrincipalDirectionBank saved to %s", bank_path)
